	...
	infoLines := []string{
		...
		formatLine(i18n.T("Info_TFLOPs"), func() string {
			...
			if maxFreq > 0 && gpuCores > 0 {
				fp32TFLOPs := gpuFP32TFLOPs(gpuCores, maxFreq)
				fp16TFLOPs := gpuFP16TFLOPs(gpuCores, maxFreq)
				if fp32TFLOPs > 0 {
		...
		func() string {
			...
			return formatLine(i18n.T("Info_ANEUsage"), fmt.Sprintf("%d%%", int(aneUtilizationPercent(lastCPUMetrics))))
		}(),
