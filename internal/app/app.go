...
func updateMainTitleWithHardware() {
	...
	gpuStr := ""
	if name := platformGPUName(); name != "" {
		gpuStr = fmt.Sprintf(" • %s", name)
	} else if info.GPUCoreCount > 0 {
	...
	mainBlock.Title = fmt.Sprintf(" lintop  •  %s  •  %s%s%s ", model, cpuStr, gpuStr, ramStr)
}
...
func Run() {
	...
	} else if envLang := firstEnv("LINTOP_LANG", "MACTOP_LANG"); envLang != "" {
...
func setupLogfile() (*os.File, error) {
	logPath := mactopStatePath("lintop.log")
...
func updatePowerChartText(cpuMetrics CPUMetrics, thermalStr string) {
	...
	if isCompactLayout() {
		...
		if hasANE() {
			...
			PowerChart.Text = fmt.Sprintf(i18n.T("Metrics_PowerChartTextNoANECompact"),
				cpuMetrics.CPUW,
				cpuMetrics.GPUW+cpuMetrics.GPUSRAMW,
				cpuMetrics.DRAMW,
				cpuMetrics.PackageW,
		...
		if hasANE() {
			...
			PowerChart.Text = fmt.Sprintf(i18n.T("Metrics_PowerChartTextNoANE"),
				cpuMetrics.CPUW,
				cpuMetrics.GPUW+cpuMetrics.GPUSRAMW,
				cpuMetrics.DRAMW,
				cpuMetrics.SystemW,
